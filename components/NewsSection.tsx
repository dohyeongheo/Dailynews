  ...
  if (news.length === 0) {
    return (
      <section className="mb-12">
        <div className="mb-6">
          <Link href={categoryUrl} className="text-2xl font-bold text-gray-900 hover:text-blue-600 transition-colors">
            {title}
          </Link>
        </div>
        <div className="bg-gray-50 border border-gray-200 rounded-lg p-8 text-center">
          <p className="text-gray-500">아직 {title}가 없습니다. 뉴스 수집 버튼을 클릭하여 뉴스를 가져오세요.</p>
        </div>
      </section>
    );
  }
