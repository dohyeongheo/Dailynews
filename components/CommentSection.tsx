...
export default function CommentSection({ newsId, initialComments, session }: CommentSectionProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      clientLog.error("Failed to post comment", error instanceof Error ? error : new Error(String(error)), { newsId });
      alert("오류가 발생했습니다.");
    } finally {
      setIsLoading(false);
    }
  ...
  const handleDelete = async (commentId: string) => {
    ...
    try {
      ...
      clientLog.error("Failed to delete comment", error instanceof Error ? error : new Error(String(error)), { commentId });
      alert("오류가 발생했습니다.");
    }
  };
  ...
  const handleUpdate = async (commentId: string) => {
    ...
    try {
      ...
      clientLog.error("Failed to update comment", error instanceof Error ? error : new Error(String(error)), { commentId });
      alert("오류가 발생했습니다.");
    } finally {
      setIsUpdating(false);
    }
